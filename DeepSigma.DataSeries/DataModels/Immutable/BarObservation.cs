...
public record class BarObservation(decimal Open, decimal Close, decimal High, decimal Low, bool IsRolled = false, bool IsSyntheticData = false) 
    : ImmutableDataModelAbstract<BarObservation, MutableBarObservation>, IImmutableDataModel<BarObservation>
{
    /// <summary>
    ...
    /// <inheritdoc/>
    public sealed override MutableBarObservation ToMutable()
    {
        return new MutableBarObservation
        {
            Open = this.Open,
            Close = this.Close,
            High = this.High,
    ...
    ///// <inheritdoc/>
    //protected sealed override BarObservation ApplyFunction(BarObservation Item2, Func<decimal, decimal, decimal> operation)
    //{
